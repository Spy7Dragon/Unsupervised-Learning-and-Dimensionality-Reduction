...
def perform_kmeans_clustering(data):
    ...
    try:
        model = cluster.KMeans(random_state=7)
        ...
        if not io_manager.is_converged(best, best_name):
            best['n_clusters'] = comparison_generation.compare_cluster_numbers(model, data,
                                                                               'n_clusters',
                                                                               intervals=5,
                                                                               interval_size=1,
                                                                               start_index=2)
        ...
        comparison_generation.display_clusters(model, data)
